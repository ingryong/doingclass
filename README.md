...
doingclass Code 주소
https://doingcode.firebaseapp.com/
...
- 많은곳에서 직접 하나하나 제작하는 하드코딩보다는 bootstrap이나 vuetify등을 사용하여 마크업을 진행하고있다.
- 크로스브라우징이나 반응형웹 적용, 팀 단위 작업에서 더 원활한 진행이 가능

### 클린코드 작업

- 코드 작업 당시 기능이 구현되기만을 위한 코드를 작성했었고, 이로인해 보기 불편한 코드가 되어 최대한 보기 좋은 코드로 전환하는 작업을 진행하기
- vuex를 활용하여 겹치는 기능이나 상태들을 통합 관리하기
